...
public class BorrowerRepository : Repository<Borrower, Guid>, IBorrowerRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (memberStatus.HasValue) query = query.Where(b => b.MemberStatus == memberStatus.Value);
    ...
    public async Task<IReadOnlyList<Borrower>> SearchByNameOrEmailAsync(string searchTerm,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(searchTerm))
    ...
    public async Task<IReadOnlyList<Borrower>> GetByMemberStatusAsync(MemberStatus status,
        CancellationToken cancellationToken = default)
    {
        return await _dbSet
            .AsNoTracking()
