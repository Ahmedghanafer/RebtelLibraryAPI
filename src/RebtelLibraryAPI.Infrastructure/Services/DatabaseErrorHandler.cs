...
public class DatabaseErrorHandler
{
    ...
    public void HandleDatabaseException(Exception exception, string entityName, string operation)
    {
        ...
        switch (exception)
        {
            ...
            case DbUpdateException updateEx when updateEx.InnerException is SqlException sqlEx:
            ...
            default:
                ...
                throw new DataAccessException(
                    $"An unexpected error occurred while accessing the database for {entityName}.");
        }
    }
    ...
    /// </summary>
    private void HandleSqlException(SqlException sqlEx, string entityName, string operation)
    {
        switch (sqlEx.Number)
        {
            case 2601: // Unique constraint violation
            case 2627: // Unique index constraint violation
                _logger.LogWarning("Unique constraint violation for {EntityName}: {Details}", entityName,
                    sqlEx.Message);
            ...
            case 547: // Foreign key constraint violation
                _logger.LogWarning("Foreign key constraint violation for {EntityName}: {Details}", entityName,
                    sqlEx.Message);
            ...
            case 208: // Invalid object name (table doesn't exist)
                _logger.LogError(sqlEx, "Database object not found for {EntityName}: {Details}", entityName,
                    sqlEx.Message);
